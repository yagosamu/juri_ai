...
**O que o gate não impede.** Um segundo job, `baseline-change`, roda em todo pull request e reprova quando um caminho protegido (`baseline.json`, `golden/golden_set.jsonl`, `indexes/**` ou `cache/queries/**`) mudou sem o rótulo `baseline-change`, nomeando os arquivos alterados e pedindo que um mantenedor revise os números antes de adicionar o rótulo. Então um PR que edita `baseline.json` junto com uma regressão, ou que muda o golden set e o baseline juntos, agora reprova a menos que um mantenedor adicione esse rótulo. Em um repositório com um único mantenedor isso é um obstáculo, não um controle de autorização: a mesma pessoa que propõe a mudança também pode adicionar o rótulo, então isso só garante um segundo olhar deliberado, não uma revisão por outra pessoa. O gate ainda verifica só o recall@10 e o mrr da config de produção; recall@1, nDCG e as outras configs não são verificados.
