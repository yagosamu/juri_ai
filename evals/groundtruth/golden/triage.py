...
def read_triage_rows(path: Path) -> list[dict]:
    """Every row of a triage file, in file order. A malformed last line is ignored with a warning.

    A crash mid-write leaves a partial final line; ignoring it lets the run resume and judge that candidate
...
def load_triage(path: Path) -> dict[str, dict]:
    rows: dict[str, dict] = {}
    for row in read_triage_rows(path):
        rows.setdefault(row["candidate_id"], row)
    return rows


def _drop_malformed_tail(path: Path) -> None:
